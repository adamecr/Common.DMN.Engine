...
namespace net.adamec.lib.common.dmn.engine.test.complex
{
    ...
    public class HitPolicyTest
    {
        ...
        public void UniqueHitPolicyTest(int? a, int? b, string c, int hitsCount, int outVariableCount)
        {
            ...
            var file = Path.Combine(dir, "dmn/hitpolicy_Unique.dmn");
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_First.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Any.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Priority.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Priority_NoOutWithAllowed.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_RuleOrder.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_OutputOrder.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_OutputOrder_NoOutWithAllowed.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectSumHitPolicyTest(int? a, int? b, int? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_Sum.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectMinHitPolicyTest(int? a, int? b, int? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_Min.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectMaxHitPolicyTest(int? a, int? b, int? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_Max.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectCountHitPolicyTest(int? a, int? b, int? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_Count.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectSumBoolHitPolicyTest(int? a, int? b, bool? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_SumBool.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectMinBoolHitPolicyTest(int? a, int? b, bool? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_MinBool.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectMaxBoolHitPolicyTest(int? a, int? b, bool? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_MaxBool.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectCountBoolHitPolicyTest()
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_CountBool.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectCountStrHitPolicyTest(int? a, int? b, int? c, int hitsCount)
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_CountStr.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        public void CollectSumStrHitPolicyTest()
        {
            ...
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_Collect_SumStr.dmn");
            var ctx = DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_WrongPolicy.dmn");
        ...
        {
            var dir = AppDomain.CurrentDomain.BaseDirectory;
            var file = Path.Combine(dir, "dmn/hitpolicy_WrongAggregation.dmn");
            Action act = () => DmnExecutionContextFactory.CreateExecutionContext(DmnParser.Parse(file));
