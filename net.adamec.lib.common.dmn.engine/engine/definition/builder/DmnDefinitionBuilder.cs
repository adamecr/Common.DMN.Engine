...
namespace net.adamec.lib.common.dmn.engine.engine.definition.builder
{
    ...
    public class DmnDefinitionBuilder
    {
        ...
        public static DmnDefinition Test() //TODO remove DmnDefinitionBuilder.Test
        {
            var definition = new DmnDefinitionBuilder()
                .Build();


            var inputDefinitions = new DmnDefinitionBuilder()
                .WithInput<int>("input1")
                .WithInput<int>("input2", out var inputVariable2)
                .WithInput("input2", typeof(string))
                .WithInput("input4", typeof(string), out var inputVariable4)
            ...
            var d = new DmnDefinitionBuilder()
                .WithInput<int>("input1")
                ...
                .WithTableDecision("tableDecision", table =>
                    table
                        ...
                        .WithRule("rule 1", r => r.When(tblInputRef, "variable1==true").Then(tblOutputRef, @"""T"""))
                        .WithRule("rule 2", r => r.Always().Then(tblOutputRef, @"""F""")))
                .Build();
            ...
            var expressionDecisionDefinition3 = new DmnDefinitionBuilder()
                ...
                .WithExpressionDecision("eDecision3", eDecision =>
                    eDecision.Put("input1>3").To(outputVariableE3)
                    )
                .Build();
            ...
            var expressionDecisionDefinition4 = new DmnDefinitionBuilder()
                ...
                .WithExpressionDecision("eDecision4", eDecision =>
                        eDecision.Put("input1>3").To(outputVariableE4),
                    out var eDecision4)
                .Build();
            ...
            var expressionDecisionDefinition5 = new DmnDefinitionBuilder()
                ...
                .WithExpressionDecision("eDecision5A", eDecision =>
                        eDecision.Put("input1>3").To(outputVariableE5),
                    out var eDecision5A)
            ...
            var tableDefinition1 = new DmnDefinitionBuilder()
                .WithInput<int>("input1", out var input1T1)
                .WithInput<int>("input2", out var input2T1)
                .WithVariable<string>("someVariableT1", out var someVariableT1)
                .WithVariable<string>("someVariableT2", out var someVariableT2)
                .WithVariable<string>("someVariableT3", out var someVariableT3)
                .WithTableDecision("tDecision1", table =>
                     table
                         .WithInput(input1T1, out var t1Input1)
                         .WithInput(input2T1, out var t1Input2, "a", "b")
                         .WithInput("input1.ToString()[0]", out var t1Input3, "1", "2", "3", "4")
                         .WithOutput(someVariableT1, out var t1Output1)
                         .WithOutput(someVariableT2, out var t1Output2, "s1", "s2", "s3")
                         .WithOutput(someVariableT3, out var t1Output3)
                         .WithHitPolicy(HitPolicyEnum.Unique)
                         .WithAggregation(CollectHitPolicyAggregationEnum.List) //redundant here as the HitPolicy=Unique
                         .WithRule("t1Rule1", r => r
                              .When(t1Input1, "")
                              .Then(t1Output1, ""))
                         .WithRule("t1Rule2", r => r
                             .When(t1Input1, "")
                             .And(t1Input2, "")
                             .Then(t1Output1, "")
                             .And(t1Output2, "")
                             .And(t1Output3, ""))
                         .WithRule("t1Rule3", "some rule descr", r => r
                              .When(t1Input1, "")
                              .And(t1Input2, "")
                              .And(t1Input3, "")
                              .Then(t1Output1, ""))
                         .WithRule("t1Rule4", r => r
                             .When(t1Input1, "")
                             .Then(t1Output1, "")
                             .And(t1Output2, "")
                             .And(t1Output3, ""))
                         .WithRule("t1Rule5", r => r
                             .When(t1Input1, "")
                             .And(t1Input2, "")
                             .Then(t1Output1, ""))
                         .WithRule("t1Rule6", r => r
                             .When(t1Input1, "")
                             .And(t1Input2, "")
                             .Then(t1Output1, "")
                             .And(t1Output2, ""))
                         .WithRule("t1Rule7", r => r
                             .When(t1Input1, "")
                             .And(t1Input2, "")
                             .Then(t1Output1, "")
                             .And(t1Output2, "")
                             .And(t1Output3, ""))
                         .WithRule("t1Rule8", r => r
                             .When(t1Input1, "")
                             .And(t1Input2, "")
                             .And(t1Input3, "")
                             .Then(t1Output1, "")
                             .And(t1Output2, "")
                             .And(t1Output3, ""))
                         .WithRule("t1Rule9", r => r
                             .Always()
                             .Then(t1Output1, ""))
                         .WithRule("t1Rule10", r => r
                             .Always()
                             .Then(t1Output1, "")
                             .And(t1Output2, ""))
                         .WithRule("t1Rule11", r => r
                             .Always()
                             .Then(t1Output1, "")
                             .And(t1Output2, "")
                             .And(t1Output3, ""))
