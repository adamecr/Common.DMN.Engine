...
namespace net.adamec.lib.common.dmn.engine.test.shared.builder
{
    [TestClass]
    ...
    public class BuilderBasicTests
    {
        ...
        public void TableAlwaysTest(int input, int output)
        {
            ...
            result.SingleResult[0].Value.Should().Be(output);
        }
        ...
        public void TableOutputsTest(int input, int? outputInt, bool? outputBool, string outputStr, int varInt, bool varBool, string varStr)
        {
            ...
            result.IsSingleResult.Should().BeTrue();
            if(outputInt!=null)result.Results[0]["output1"].Value.Should().Be(outputInt);
            if (outputBool != null) result.Results[0]["output2"].Value.Should().Be(outputBool);
            if (outputStr != null) result.Results[0]["output3"].Value.Should().Be(outputStr);
